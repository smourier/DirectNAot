...
public partial interface IWICComponentInfo
{
    ...
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetAuthor(uint cchAuthor, [MarshalUsing(CountElementName = nameof(cchAuthor))] PWSTR wzAuthor, out uint pcchActual);
    ...
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetVersion(uint cchVersion, [MarshalUsing(CountElementName = nameof(cchVersion))] PWSTR wzVersion, out uint pcchActual);
    
    // https://learn.microsoft.com/windows/win32/api/wincodec/nf-wincodec-iwiccomponentinfo-getspecversion
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetSpecVersion(uint cchSpecVersion, [MarshalUsing(CountElementName = nameof(cchSpecVersion))] PWSTR wzSpecVersion, out uint pcchActual);
    
    // https://learn.microsoft.com/windows/win32/api/wincodec/nf-wincodec-iwiccomponentinfo-getfriendlyname
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetFriendlyName(uint cchFriendlyName, [MarshalUsing(CountElementName = nameof(cchFriendlyName))] PWSTR wzFriendlyName, out uint pcchActual);
}
