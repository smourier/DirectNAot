...
public partial interface ID2D1SvgStrokeDashArray : ID2D1SvgAttribute
{
    ...
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetDashes([MarshalUsing(CountElementName = nameof(dashesCount))] out D2D1_SVG_LENGTH[] dashes, uint dashesCount, uint startIndex);
    
    // https://learn.microsoft.com/windows/win32/Direct2D/id2d1svgstrokedasharray-getdashes-overload
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetDashes([MarshalUsing(CountElementName = nameof(dashesCount))] out float[] dashes, uint dashesCount, uint startIndex);
