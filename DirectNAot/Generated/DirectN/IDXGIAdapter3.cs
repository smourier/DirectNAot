...
public partial interface IDXGIAdapter3 : IDXGIAdapter2
{
    ...
    HRESULT RegisterHardwareContentProtectionTeardownStatusEvent(HANDLE hEvent, out uint pdwCookie);
    ...
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT QueryVideoMemoryInfo(uint NodeIndex, DXGI_MEMORY_SEGMENT_GROUP MemorySegmentGroup, out DXGI_QUERY_VIDEO_MEMORY_INFO pVideoMemoryInfo);
    ...
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT RegisterVideoMemoryBudgetChangeNotificationEvent(HANDLE hEvent, out uint pdwCookie);
