...
public partial interface IWMCodecStrings
{
    ...
    HRESULT GetName(ref DMO_MEDIA_TYPE pmt, uint cchLength, [MarshalUsing(CountElementName = nameof(cchLength))] PWSTR szName, ref uint pcchLength);
    
    // https://learn.microsoft.com/windows/win32/api/wmcodecdsp/nf-wmcodecdsp-iwmcodecstrings-getdescription
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetDescription(ref DMO_MEDIA_TYPE pmt, uint cchLength, [MarshalUsing(CountElementName = nameof(cchLength))] PWSTR szDescription, ref uint pcchLength);
}
