...
public partial interface ID3D12Device1 : ID3D12Device
{
    ...
    public HRESULT CreatePipelineLibrary(nint pLibraryBlob, nuint BlobLength, in Guid riid, out nint ppPipelineLibrary);
    
    // https://learn.microsoft.com/windows/win32/api/d3d12/nf-d3d12-id3d12device1-seteventonmultiplefencecompletion
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    public HRESULT SetEventOnMultipleFenceCompletion(in ID3D12Fence ppFences, in ulong pFenceValues, uint NumFences, D3D12_MULTIPLE_FENCE_WAIT_FLAGS Flags, HANDLE hEvent);
    
    // https://learn.microsoft.com/windows/win32/api/d3d12/nf-d3d12-id3d12device1-setresidencypriority
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    public HRESULT SetResidencyPriority(uint NumObjects, in ID3D12Pageable ppObjects, in D3D12_RESIDENCY_PRIORITY pPriorities);
}
