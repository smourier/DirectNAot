...
public partial interface IMFSensorActivityReport
{
    ...
    HRESULT GetFriendlyName([MarshalUsing(CountElementName = nameof(cchFriendlyName))] PWSTR FriendlyName, uint cchFriendlyName, out uint pcchWritten);
    
    // https://learn.microsoft.com/windows/win32/api/mfidl/nf-mfidl-imfsensoractivityreport-getsymboliclink
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetSymbolicLink([MarshalUsing(CountElementName = nameof(cchSymbolicLink))] PWSTR SymbolicLink, uint cchSymbolicLink, out uint pcchWritten);
