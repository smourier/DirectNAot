...
public partial interface IWICJpegFrameDecode
{
    ...
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetScanHeader(uint scanIndex, out WICJpegScanHeader pScanHeader);
    
    // https://learn.microsoft.com/windows/win32/api/wincodec/nf-wincodec-iwicjpegframedecode-copyscan
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT CopyScan(uint scanIndex, uint scanOffset, uint cbScanData, nint /* byte array */ pbScanData, out uint pcbScanDataActual);
    
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT CopyMinimalStream(uint streamOffset, uint cbStreamData, nint /* byte array */ pbStreamData, out uint pcbStreamDataActual);
}
