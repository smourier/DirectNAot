...
public partial interface IDWriteTextAnalyzer2 : IDWriteTextAnalyzer1
{
    ...
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetTypographicFeatures([MarshalUsing(typeof(UniqueComInterfaceMarshaller<IDWriteFontFace>))] IDWriteFontFace fontFace, DWRITE_SCRIPT_ANALYSIS scriptAnalysis, PWSTR localeName, uint maxTagCount, out uint actualTagCount, [In][Out][MarshalUsing(CountElementName = nameof(maxTagCount))] DWRITE_FONT_FEATURE_TAG[] tags);
    
    // https://learn.microsoft.com/windows/win32/api/dwrite_2/nf-dwrite_2-idwritetextanalyzer2-checktypographicfeature
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT CheckTypographicFeature([MarshalUsing(typeof(UniqueComInterfaceMarshaller<IDWriteFontFace>))] IDWriteFontFace fontFace, DWRITE_SCRIPT_ANALYSIS scriptAnalysis, PWSTR localeName, DWRITE_FONT_FEATURE_TAG featureTag, uint glyphCount, [In][MarshalUsing(CountElementName = nameof(glyphCount))] ushort[] glyphIndices, nint /* byte array */ featureApplies);
}
