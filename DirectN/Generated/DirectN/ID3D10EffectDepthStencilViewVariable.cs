...
public partial interface ID3D10EffectDepthStencilViewVariable : ID3D10EffectVariable
{
    ...
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetDepthStencil([MarshalUsing(typeof(UniqueComInterfaceMarshaller<ID3D10DepthStencilView>))] out ID3D10DepthStencilView ppResource);
    ...
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT GetDepthStencilArray([In][Out][MarshalUsing(CountElementName = nameof(Count))] ID3D10DepthStencilView[] ppResources, uint Offset, uint Count);
}
