...
public partial interface ID2D1Transform : ID2D1TransformNode
{
    ...
    HRESULT MapOutputRectToInputRects(in RECT outputRect, [In][Out][MarshalUsing(CountElementName = nameof(inputRectsCount))] RECT[] inputRects, uint inputRectsCount);
    ...
    [PreserveSig]
    [return: MarshalAs(UnmanagedType.Error)]
    HRESULT MapInputRectsToOutputRect([In][MarshalUsing(CountElementName = nameof(inputRectCount))] RECT[] inputRects, [In][MarshalUsing(CountElementName = nameof(inputRectCount))] RECT[] inputOpaqueSubRects, uint inputRectCount, out RECT outputRect, out RECT outputOpaqueSubRect);
