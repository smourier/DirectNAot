...
public static class MessageDecoder
{
    ...
    private static ConcurrentDictionary<uint, string[]> GetMessageNames()
    {
        var dic = new Dictionary<uint, List<string>>();
        foreach (var msg in typeof(MessageDecoder).GetFields(BindingFlags.Static | BindingFlags.Public).Where(m => m.IsLiteral && m.Attributes.HasFlag(FieldAttributes.HasDefault) && m.FieldType == typeof(uint)))
        {
            var key = (uint)msg.GetValue(null)!;
...
#pragma warning restore IDE0306 // Simplify collection initialization
    ...
    public static string Decode(HWND handle, uint msg, WPARAM wParam, LPARAM lParam, nint? lResult = null)
    {
        var id = Parenthesize(MsgToString(msg));
        var description = string.Empty;
        if (msg == WM_PARENTNOTIFY)
        {
            description = Parenthesize(MsgToString(LOWORD(wParam.Value)));
        }

        var s = "msg=0x" + Convert.ToString(msg, 16) + " (" + msg + ")" + id
        + " hwnd=0x" + Convert.ToString(handle.Value, 16)
        + " wparam=0x" + Convert.ToString((long)wParam.Value, 16)
        + " lparam=0x" + Convert.ToString(lParam.Value, 16) + description;

        if (lResult.HasValue)
        {
            s += " result=0x" + Convert.ToString(lResult.Value, 16);
        }
        return s;
    }
    ...
    private static uint LOWORD(uint n) => n & 0xffff;
    private static uint LOWORD(nuint n) => LOWORD((uint)(long)n);
    ...
    public static string? MsgToString(uint msg)
    {
